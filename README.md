...
**NAT → IPAM:** ورودی‌های **VIP** در FortiGate به آدرس‌های IPAM برای IP خارجی (`extip`) تبدیل می‌شوند با فیلد بومی **`nat_inside`** در NetBox که به آدرس سرور داخلی نگاشت‌شده اشاره دارد؛ **poolهای IP** به آدرس‌های ساده IPAM برای محدوده SNAT تبدیل می‌شوند. آدرس‌های NAT علامت‌دار (`netbox-sync: nat …`) که دیگر گزارش نشوند پس از هر اجرا حذف می‌شوند؛ آدرس‌های دستی هرگز دست نمی‌خورند. نکته: چند VIP ممکن است یک `extip` مشترک داشته باشند (به‌ازای هر آدرس یک رکورد — توضیح یکی از VIPهای استفاده‌کننده نمایش داده می‌شود).
